...
export default function CartClient() {
  ...
  const handleClearCart = async () => {
    try {
      await clearCart().unwrap(); // ✅ No arguments passed
      toast.success("Items cleared successfully"); // ✅ Fixed typo
    } catch {
      ...
      <div className="grid md:grid-cols-[1fr_360px] gap-10">
        ...
        <aside className="rounded-2xl border p-6 h-fit">
          ...
          <button
            ...
            className="w-full py-3 mt-2 rounded-xl bg-red-500 hover:bg-red-600 disabled:bg-red-400 disabled:cursor-not-allowed text-white font-semibold flex items-center justify-center gap-2 transition-all duration-200"
          >
            {clearCartLoading ? (
              <>
                <svg
